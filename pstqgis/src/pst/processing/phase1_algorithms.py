...
class AngularIntegrationAlgorithm(PstProcessingAlgorithmBase):
	...
	def checkParameterValues(self, parameters, context):
		if not self.parameterAsBool(parameters, self.CALC_NO_WEIGHT, context) and not self.parameterAsBool(parameters, self.CALC_LENGTH_WEIGHT, context):
			return (False, 'Please select at least one weight mode.')
		if not self.parameterAsBool(parameters, self.NORM_NORMALIZATION, context) and not self.parameterAsBool(parameters, self.NORM_SYNTAX, context) and not self.parameterAsBool(parameters, self.NORM_HILLIER, context):
...
class AngularChoiceAlgorithm(PstProcessingAlgorithmBase):
	...
	def checkParameterValues(self, parameters, context):
		if not self.parameterAsBool(parameters, self.WEIGHT_NONE, context) and not self.parameterAsBool(parameters, self.WEIGHT_LENGTH, context):
			return (False, 'Please select at least one weight mode.')
		if not self.parameterAsBool(parameters, self.NORM_NONE, context) and not self.parameterAsBool(parameters, self.NORM_NORMALIZATION, context) and not self.parameterAsBool(parameters, self.NORM_STANDARD, context) and not self.parameterAsBool(parameters, self.NORM_SYNTAX, context):
			return (False, 'Please select at least one normalization mode.')
		props = self._collectProperties(parameters, context)
		if not props['rad_straight_enabled'] and not props['rad_walking_enabled'] and not props['rad_steps_enabled'] and not props['rad_angular_enabled']:
			return (False, 'Please specify at least one radius.')
